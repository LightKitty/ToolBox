...
namespace XMindAPI.LIB
{
    ...
    public enum XMindMarker
    {
        ...
        ArrowUpLeft,
        [Description("arrow-refresh")]
        ArrowRefresh,
        // Symbol markers:
        [Description("symbol-right")]
        SymbolRight,
        [Description("symbol-attention")]
        SymbolAttention,
        [Description("c_symbol_pen")]
        CSymbolPen,
        // Other markers:
    ...
    public class XMindTopic
    {
        public string ID { get; private set; }
        public string Name { get; private set; }
        public List<XMindMarker> Markers { get; private set; }
        public string AttachmentUrl { get; private set; }

        public XMindTopic Parent { get; set; }

        public List<XMindTopic> Topics { get; set; }
        ...
        internal XMindTopic(XMindTopic parent, string id, string name, List<XMindMarker> markers, string attachmentUrl) : this()
        {
            Parent = parent;
            ID = id;
            Name = name;
    ...
    public class XMindWorkBook
    {
        ...
        public XMindWorkBook(Stream _stream)
        {
            _defaultContentNS = XNamespace.Get("urn:xmind:xmap:xmlns:content:2.0");
            _defaultManifestNS = XNamespace.Get("urn:xmind:xmap:xmlns:manifest:1.0");
            _defaultMetaNS = XNamespace.Get("urn:xmind:xmap:xmlns:meta:2.0");
        ...
        public Dictionary<string, byte[]> attachments = new Dictionary<string, byte[]>();
        ...
        public List<XMindSheet> GetSheetInfo()
        {
            ...
            foreach (XElement el in GetSheets())
            {
                ...
                List<XMindMarker> makers = GetTopicMarkers(GetTopic(centralTopicId));

                string imgUrl = GetTopicImgUrl(GetTopic(centralTopicId));
                ...
                XMindTopic xmTopic = new XMindTopic(null, centralTopicId, centralTopicTitle, makers, imgUrl);
        ...
        public string GetTopicTitle(string topicId)
        {
            ...
            if (topic != null)
            {
                IEnumerable<string> titles = topic.Descendants().Where(w => w.Name.ToString().EndsWith("title")).Select(s => s.Value);
                if (titles.Count() > 0) title = titles.First();
            }
        ...
        /// 获取topic的markers
        /// </summary>
        /// <param name="topicId">Topic's ID</param>
        /// <returns>markers列表</returns>
        public List<XMindMarker> GetTopicMarkers(string topicId)
        {
            XElement topic = GetTopic(topicId);
            if (topic == null)
            {
            ...
            List<XMindMarker> markers = GetTopicMarkers(topic);
        ...
        /// <returns></returns>
        public string GetTopicImgUrl(XElement topic)
        {
            XElement xhtmlImg = topic.Elements().Where(w => w.Name.ToString().EndsWith("img")).FirstOrDefault();
            if (xhtmlImg != null&& xhtmlImg.Attributes().Any(w => w.Name.ToString().EndsWith("src")))
            {
                string imgUrl = xhtmlImg.Attributes().FirstOrDefault(w => w.Name.ToString().EndsWith("src")).Value;
                if (imgUrl.StartsWith("xap:"))
                {
        ...
        private void GetTopicsRecursively(XElement parent, XMindSheet xmSheet, XMindTopic xmTopic)
        {
                ...
                && GetAttribValue(w1.Parent.Parent.Parent, "id") == GetAttribValue(parent, "id")))
            {
                ...
                XMindTopic nextXmTopic = new XMindTopic(xmTopic, topicId, GetTopicTitle(topicId), GetTopicMarkers(GetTopic(topicId)), GetTopicImgUrl(GetTopic(topicId)));
        ...
        /// <returns></returns>
        private string StreamToString(Stream _stream)
        {
